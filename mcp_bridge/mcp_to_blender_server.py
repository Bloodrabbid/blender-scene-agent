...
def timer_internal_vars_calc(
    ...
    _timer.idle_countdown_reset = math.ceil(
        _timer.interval_idle_delay / _timer.interval_active
    )
    _timer.idle_countdown = _timer.idle_countdown_reset
    _timer.client_timeout_countdown = max(
        2, math.ceil(_CLIENT_TIMEOUT / _timer.interval_active)
    )
...
def _send_response(conn: socket.socket, response: dict[str, object]) -> None:
    """Send a complete response even when the accepted client was non-blocking.

    Large screenshot payloads can exceed the loopback socket buffer. Calling
    ``sendall`` on the non-blocking interactive socket may then write only a
    prefix before raising ``BlockingIOError``, producing truncated JSON at the
    MCP client. Responses are bounded to 10 MiB, so a short blocking send on
    localhost is safe and guarantees protocol framing.
    """
    ...
    if not isinstance(result, dict):
        ...
        if strict_json:
            try:
                ...
                response = {
                    "status": "error",
                    "message": "The `result` value is not JSON-serializable: {:s}".format(
                        str(ex)
                    ),
                }
            else:
...
def _execute_code_from_request(
    ...
    if use_log:
        if exec_result.check_fn is not None:
            ...
            print(
                "response: {:s}".format(json.dumps(exec_result.response, indent=2)),
                file=sys.stderr,
            )
    ...
    for client in _state.clients[:]:
        ...
        if client.timeout <= 0:
            try:
                ...
                _send_response(client.conn, err)
            except OSError:
                pass
        ...
        if len(client.buffer) > _MAX_REQUEST_BYTES:
            try:
                err = {
                    "status": "error",
                    "message": "Request exceeds {:d} byte limit".format(
                        _MAX_REQUEST_BYTES
                    ),
                }
                _send_response(client.conn, err)
        ...
        if exec_result.check_fn is not None:
            ...
            try:
                _send_response(client.conn, exec_result.response)
            except OSError:
                pass
            _close_client(client)
        did_work = True
            ...
            if exec_result.check_fn is not None:
                ...
                response = {
                    **exec_result.response,
                    "status": "error",
                    "message": _DEFERRED_UNSUPPORTED_MESSAGE,
                }
