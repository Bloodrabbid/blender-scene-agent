...
from .mcp_to_blender_server import _send_response
...
def _send_and_close(dc: _DeferredClient, response: dict[str, object]) -> None:
    try:
        _send_response(dc.conn, response)
    except OSError:
        pass
    try:
        dc.conn.close()
    ...
    for dc in _deferred_clients[:]:
        ...
        if time.monotonic() > dc.deadline:
            _send_and_close(
                ...
                {
                    "status": "error",
                    "message": "Deferred operation timed out after {:.0f} seconds".format(
                        _DEFERRED_TIMEOUT
                    ),
                },
            )
        ...
        if dc.strict_json:
            try:
                ...
                _send_and_close(
                    ...
                    {
                        "status": "error",
                        "message": "Deferred result is not JSON-serializable: {:s}".format(
                            str(ex)
                        ),
                    },
                )
